                                          ...
                                          WINDOW_ORDER_INFO* orderInfo)
{
	...
	if (orderInfo->fieldFlags & WINDOW_ORDER_ICON)
	{
		...
		if (result)
		{
			WLog_Print(up->log, WLOG_DEBUG, "WindowIcon windowId=%" PRIu32 "", orderInfo->windowId);
	...
	else if (orderInfo->fieldFlags & WINDOW_ORDER_CACHED_ICON)
	{
		...
		if (result)
		{
			WLog_Print(up->log, WLOG_DEBUG, "WindowCachedIcon windowId=%" PRIu32 "",
			           orderInfo->windowId);
	...
	else if (orderInfo->fieldFlags & WINDOW_ORDER_STATE_DELETED)
	{
		...
		WLog_Print(up->log, WLOG_DEBUG, "WindowDelete windowId=%" PRIu32 "", orderInfo->windowId);
                                   ...
                                   const MONITORED_DESKTOP_ORDER* monitored)
{
	...
	if (orderInfo->fieldFlags & WINDOW_ORDER_FIELD_DESKTOP_ACTIVE_WND)
		DUMP_APPEND(buffer, bufferSize, " activeWindowId=%" PRIu32 "", monitored->activeWindowId);
	...
	if (orderInfo->fieldFlags & WINDOW_ORDER_FIELD_DESKTOP_ZORDER)
	{
		...
		{
			DUMP_APPEND(buffer, bufferSize, "%" PRIu32 ",", monitored->windowIds[i]);
		}
		DUMP_APPEND(buffer, bufferSize, ")");
	}
	WLog_Print(log, WLOG_DEBUG, buffer);
                                           ...
                                           WINDOW_ORDER_INFO* orderInfo)
{
	...
	if (orderInfo->fieldFlags & WINDOW_ORDER_FIELD_DESKTOP_NONE)
	{
		...
		WLog_Print(up->log, WLOG_DEBUG, "NonMonitoredDesktop, windowId=%" PRIu32 "",
		           orderInfo->windowId);
